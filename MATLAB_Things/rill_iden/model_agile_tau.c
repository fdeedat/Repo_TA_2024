   ...
   )
{
    ...
    */
    ...
    double *Ixx, *Iyy, *Izz, *Krx, *Kry, *Krz, *Jr;
    ...
    dx[1] = x[4];
    dx[2] = x[5];
    dx[3] = ((Iyy[0]-Izz[0])/Ixx[0])*dx[1]*dx[2]+ u[0]/Ixx[0]- Krx[0]*(dx[0]/Ixx[0])+ Jr[0]*dx[1]*(u[2])/Ixx[0];
    dx[4] = ((Izz[0]-Ixx[0])/Iyy[0])*dx[0]*dx[2]+ u[1]/Iyy[0]- Kry[0]*(dx[1]/Iyy[0])- Jr[0]*dx[0]*(u[2])/Iyy[0];
    dx[5] = ((Ixx[0]-Iyy[0])/Izz[0])*dx[1]*dx[0]+ u[2]/Izz[0]- Krz[0]*dx[2]/Izz[0];
}
