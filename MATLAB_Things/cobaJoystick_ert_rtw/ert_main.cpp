...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(cobaJoystick_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(cobaJoystick_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(cobaJoystick_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(cobaJoystick_M, 0);
  ...
  // Initialize model
  cobaJoystick_initialize();

  // External Mode initialization
  errorCode = extmodeInit(cobaJoystick_M->extModeInfo, &rtmGetTFinal
    (cobaJoystick_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(cobaJoystick_M, true);
    }
  }
